...
void main() {
	...
	vec2 voxelTextureUV = vec2(
		...
		vec4 diffuse = texture2D(u_diffuseTexture, voxelTextureUV) * u_diffuseColor;
	#elif defined(diffuseTextureFlag) && defined(colorFlag)
		vec4 diffuse = texture2D(u_diffuseTexture, voxelTextureUV) * v_color;
	#elif defined(diffuseTextureFlag)
		vec4 diffuse = texture2D(u_diffuseTexture, voxelTextureUV);
